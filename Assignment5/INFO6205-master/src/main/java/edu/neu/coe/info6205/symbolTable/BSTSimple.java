...
//    }
	...
	private Value get(Node node, Key key) {
		...
		if (cf < 0)
			return get(node.smaller, key);
		else if (cf > 0)
			return get(node.larger, key);
		else
			return node.value;
	}
	...
	private NodeValue put(Node node, Key key, Value value) {
		...
		if (cf == 0) {
			...
			// if key is less than node's key, we recursively invoke put in the smaller
			// subtree
			...
			// if key is greater than node's key, we recursively invoke put in the larger
			// subtree
	...
	class Node {
		...
		public String toString() {
			...
			if (smaller != null)
				sb.append(", smaller: " + smaller.key);
			if (larger != null)
				sb.append(", larger: " + larger.key);
			return sb.toString();
		}
